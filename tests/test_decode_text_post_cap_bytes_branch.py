...
``null_pos > max_size`` re-check (types.py:362, 369) was uncovered.

Pin both:
- A bytes payload whose NUL lands within the scan window but past
  ``max_size`` raises with a length-cited ``DecodeError``.
- The same shape via the post-window ``null_pos > max_size`` check at
  the function tail (line 369) is exercised by the existing chunked
  path tests; here we assert the bytes path emits the same error.
"""
...
def test_bytes_branch_post_cap_check_rejects() -> None:
    ...
    check at line 362 short-circuits BEFORE the UTF-8 decode."""
