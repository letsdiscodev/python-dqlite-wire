"""Pin: ``NodeInfo`` rejects unknown role values at construction
time, matching the symmetric ``ServersResponse.decode_body``
``unknown_role_policy="reject"`` behaviour.

The decoder fences invalid roles (raises ``DecodeError`` with the
``unknown_role_policy="reject"`` default) but the encoder side
historically called ``encode_uint64(node.role)`` with no validation.
A user constructing ``NodeInfo(node_id=1, address="...", role=999)``
would silently emit ``999`` onto the wire; a peer-side decoder
would then reject it (correct), but the diagnostic surfaces on the
consumer side rather than at construction time.

The fix adds ``NodeInfo.__post_init__`` that rejects role values
outside the canonical ``NodeRole`` enum. Mirrors
``AssignRequest.__post_init__`` which already does the same
coercion + validation for the request-side encoder.
"""
...
def test_node_info_accepts_canonical_roles(role: NodeRole | int, expected: NodeRole) -> None:
    ...
    # Internal representation is the enum, regardless of input shape.
...
def test_node_info_rejects_unknown_roles(bogus_role: int) -> None:
    """Bogus role values in uint64 range must NOT reach the wire.
...
def test_node_info_rejects_out_of_range_roles(out_of_range: int) -> None:
    """Out-of-uint64-range role values are rejected with the
    documented uint64 diagnostic (``EncodeError``), distinguishing
    "value cannot be encoded at all" from "value encodable but not a
    known role." Mirrors the prefilter ordering in
    ``AssignRequest.__post_init__``.
    """
    with pytest.raises(EncodeError, match="role"):
